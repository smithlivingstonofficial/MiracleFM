...
import { ArrowLeft, Upload, Image as ImageIcon, Loader2, Sparkles, Disc, Grid, User, Music } from "lucide-react";
...
export default function TrackEditor() {
  ...
  const [form, setForm] = useState({ title: "", artist_id: "", album_id: "", lyrics: "", cover_url: "" });
  const [originalCover, setOriginalCover] = useState(""); // Track initial state
  ...
  const save = async () => {
    ...
    try {
      ...
      // 2. Smart Cleanup: If cover changed/removed, check if old one needs deletion
      if (originalCover && originalCover !== form.cover_url) {
        // We use POST here to trigger the "Check Usage & Delete if 0" logic
  ...
  const openCoverLibrary = async () => {
    ...
    if (trackCovers) {
      trackCovers.forEach((c: any) => libraryMap.set(c.cover_url, { url: c.cover_url, type: 'track', usage_count: c.usage_count }));
    }
    // Add Artist/Album covers to selection
    if (form.artist_id) {
  ...
  return (
    <>
            ...
            <div className="group relative aspect-square rounded-[3rem] overflow-hidden bg-panel border-2 border-dashed border-white/10 hover:border-brand/40 transition-all shadow-2xl flex items-center justify-center">
              {form.cover_url ? (
                ...
                <div className="flex flex-col items-center">
                  <Music size={64} className="text-zinc-800 mb-4" />
                  <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">No Selection</p>
                </div>
              )}
              <label className="absolute inset-0 bg-brand/90 backdrop-blur-sm flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 cursor-pointer">
                <input type="file" hidden accept="image/*" onChange={handleFileUpload} />
                {uploadingImage ? <Loader2 size={32} className="text-white animate-spin mb-2" /> : <Upload size={32} className="text-white mb-2" />}
                <span className="font-black text-xs uppercase tracking-widest text-white">Upload New</span>
              </label>
            </div>
            
            <div className="space-y-3">
                <button onClick={openCoverLibrary} className="w-full flex items-center justify-center gap-3 py-5 rounded-[1.5rem] bg-panel border border-white/10 hover:border-brand/50 text-sm font-black uppercase tracking-widest transition-all shadow-lg active:scale-[0.98]">
                    <Grid size={18} className="text-brand" />
                    Asset Library
                </button>
                <button onClick={() => setForm({...form, cover_url: ""})} className="w-full py-3 text-xs font-bold text-zinc-500 hover:text-red-500 transition-colors">
                    Remove Artwork
                </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            <div className="bg-panel rounded-[3rem] p-10 border border-white/5 space-y-10">
              <div className="space-y-4">
                ...
                <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-6 text-xl font-bold text-white outline-none focus:border-brand transition-all" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <select value={form.artist_id} onChange={e => setForm({ ...form, artist_id: e.target.value, album_id: "" })} className="w-full bg-black border border-white/10 rounded-2xl p-6 font-bold text-white outline-none focus:border-brand appearance-none cursor-pointer">
                    <option value="">No Artist (Unassigned)</option>
            ...
            <textarea rows={12} value={form.lyrics} onChange={e => setForm({...form, lyrics: e.target.value})} className="w-full bg-panel border border-white/10 rounded-[2.5rem] p-10 outline-none focus:border-brand transition-all resize-none text-zinc-400 leading-loose text-base font-medium" placeholder="Lyrics..." />
          </div>
        </div>
      </div>
      ...
      {/* Library Modal */}
      {isLibraryOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in">
          <div className="bg-panel border border-white/10 rounded-[3rem] shadow-2xl max-w-5xl w-full flex flex-col max-h-[85vh]">
            <div className="p-8 border-b border-white/5 flex justify-between items-center">
              <h2 className="text-2xl font-black tracking-tight">Asset Selector</h2>
            ...
            <div className="p-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 overflow-y-auto">
              {coverLibrary.map(item => (
                <div key={item.url} onClick={() => selectCoverFromLibrary(item.url)} className="group relative aspect-square rounded-[1.5rem] overflow-hidden cursor-pointer border-2 border-transparent hover:border-brand transition-all">
                  <Image src={item.url} alt="" fill className="object-cover" />
                  {item.type !== 'track' && <div className="absolute top-3 left-3 bg-black/80 px-2 py-1 rounded-md text-[8px] font-black uppercase text-white">{item.type}</div>}
                </div>
              ))}
            </div>
          </div>
